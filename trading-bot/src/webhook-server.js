 ...
 *
 * Provides HTTP endpoints so n8n can:
 *   - Receive signals from the bot (bot → n8n)
 *   - Send commands back to the bot (n8n → bot)
 *   - Query bot status, positions, levels
 *
 * Listens on WEBHOOK_PORT (default 3001), only on localhost by default.
 */
...
class WebhookServer {
  ...
  async _handleRequest(req, res) {
    ...
    try {
      if (req.method === 'GET' && url === '/status') {
        return this._sendJson(res, await this._getStatus());
      }
      if (req.method === 'GET' && url === '/positions') {
        return this._sendJson(res, this._getPositions());
      }
      if (req.method === 'GET' && url === '/levels') {
        return this._sendJson(res, await this._getLevels(req));
      }
      ...
      if (req.method === 'GET' && url === '/stats') {
        return this._sendJson(res, this._getStats());
      }
      if (req.method === 'GET' && url === '/trades') {
        return this._sendJson(res, this._getTrades(req));
      }
  ...
  async _getStatus() {
    ...
    try {
      balance = await this.bot.exchange.fetchBalance();
    } catch (_) {}
    return {
      running: this.bot.running,
      positions: this.bot.positions.size,
      balance,
      uptime: process.uptime(),
      aiEnabled: this.bot.aiFilter ? this.bot.aiFilter.enabled : false,
    };
  }
  ...
  async _handleCommand(body) {
    ...
    switch (command) {
      case 'pause':
        this.bot.paused = true;
        logger.info('Bot paused via webhook');
        return { ok: true, message: 'Bot paused' };

      case 'resume':
        this.bot.paused = false;
        logger.info('Bot resumed via webhook');
      ...
      case 'daily_report': {
        const stats = this.bot.tradeStore.getStats();
        await this.bot.notifier.notifyDailySummary(stats);
      ...
      case 'close_all':
        for (const [key, pos] of this.bot.positions) {
          ...
          try {
            await this.bot.exchange.closePosition(pair, pos.side, pos.size);
            this.bot.riskManager.removePosition(key);
            this.bot.positions.delete(key);
            logger.info(`Closed ${key} via webhook`);
          } catch (err) {
  ...
  _handleAIOverride(body) {
    // n8n can override AI filter decision
  ...
  _getStats() {
    if (this.bot.tradeStore) {
      return this.bot.tradeStore.getStats();
    }
  ...
  _getTrades(req) {
    ...
    if (this.bot.tradeStore) {
      return { trades: this.bot.tradeStore.getRecentTrades(limit) };
    }
    return { trades: [] };
  }

  // ── Outbound: push events to n8n ──────────────
    ...
    return new Promise((resolve) => {
      ...
      const req = transport.request(
        ...
        (res) => {
          ...
          res.on('end', () => {
            logger.debug(`n8n webhook response: ${res.statusCode}`);
            resolve(body);
          });
        }
      );
